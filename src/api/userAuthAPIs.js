	...
	fetch('http://localhost:8080/api/auth/signin', {
		...
		response.json().then((json) => {
			if(response.ok) {
				...
				promiseRejectRef({
					reason: "Server error occurred, Unable to login. Please try again.",
					response: response,
				});
			}
		}).catch((error) => {
		...
		promiseRejectRef({
			reason: "Error occurred, unable to connect to the server. Please check your internet connection and try again.",
			response: err,
		});
	});
	return promise;
